...
if [ -z "${ADDITIONAL_APT_REPOS}" ]; then
  echo "No additional APT repo, skipping"
  exit 0
fi
...
BEGIN {RS=";";FS=","}
{
  ...
  Repo = "/etc/apt/sources.list.d/"vars["repo"]".list"
}
{cmd="sudo rm -rf " Repo; system(cmd)}
'

sudo awk "$AWK_CMD" <<< "${ADDITIONAL_APT_REPOS}"
